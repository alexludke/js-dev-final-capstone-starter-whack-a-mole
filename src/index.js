...
function setDelay(difficulty) {
  ...
  // Else statement returns a random number between 600 and 1200 for any difficulty besides easy and normal.
  if (difficulty === "easy") {
    return 1500;
...
function chooseHole(holes) {
  ...
  // gets the hole associated with the random index
  const hole = holes[index];

  // checks if the selected hole is the same as the last selected hole
  if (hole === lastHole) {
    return chooseHole(holes);
  }

  // updates last hole with selected hole
  lastHole = hole;
